...
> 文档位于三项目**共享文档仓 `teyvat-docs`**：`doc/` 与 `shared-docs/` 是 Junction 链接（不进本仓，已在 `.gitignore` 忽略）。
> 修改文档请到 `C:\Users\HONOR\Desktop\teyvat-docs` 提交；链接丢失时运行该仓的 `setup-docs.ps1` 重建。

| # | 文档 | 什么时候看 |
| --- | --- | --- |
| 01 | [接口契约](shared-docs/01-接口契约.md) | 对接前必读：字段、错误码、SSE 协议、兼容性承诺 |
| 02 | [AI 派蒙架构设计（DeepSeek）](doc/02-AI派蒙架构设计-DeepSeek方案.md) | 想了解设计取舍、人格与上下文 |
| 03 | [主服务对接说明](shared-docs/03-主服务对接说明.md) | 主服务侧的改动清单与排障（F1/F2 已完成） |
